...
def run_test_case(A, b):
    ...
    solver.initialize(A)
    x = solver.solve(A[0], b)
    residual_norm = solver.compute_residual_norm(A[0], x, b)
    ...
    loaded_x = loaded_solver.solve(A[0],b)
    loaded_residual_norm = loaded_solver.compute_residual_norm(A[0], loaded_x, b)
...
# Test Case 11. Multiple matrix A, and multiple right-hand side b
A = np.array([[[1, 2], [3, 4], [5, 6]],[[1, 2], [1, 2], [1, 2]]])
...
for b in b_list:
    x = solver.solve(np.array([[1, 2], [3, 4], [5, 6]]), b)
    solutions.append(x)
    print(solver.check_solution(np.array([[1, 2], [3, 4], [5, 6]]), x, b))
# Print solutions
